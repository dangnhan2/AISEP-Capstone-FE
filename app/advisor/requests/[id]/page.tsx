...
import { IMentorshipReport } from "@/types/startup-mentorship";
import {
  GetAdvisorProfile,
  GetAdvisorMentorshipById,
...
const TIMELINE_LABELS: Record<string, string> = {
  ...
  REQUEST_SUBMITTED: "Yêu cầu được tạo",
...
const handleAcceptConfirm = async () => {
  ...
  const handleRejectConfirm = async () => {
    ...
    try {
      const res = await RejectMentorshipRequest(requestId, rejectReason.trim());
      if (res.isSuccess || res.success) {
        setRequest({
          ...request,
          status: "REJECTED",
        ...
        toast.error(res.message || "Không thể tải lên thay đổi");
      }
    } catch (err) {
      toast.error("Có tiếng phát sinh nội bộ (API lỗi)");
    }
  };
...
const handleCancelConfirm = async () => {
    ...
    try {
      const res = await CancelMentorshipRequest(requestId, cancelReason.trim());
      if (res.isSuccess || res.success) {
        setRequest({
          ...request,
          status: "CANCELLED",
        ...
        toast.error(res.message || "Lỗi khi huỷ buổi tư vấn");
      }
    } catch (err: any) {
      const status = err?.response?.status ?? err?.status;
      if (status === 403) {
        toast.error("Chức năng hủy không được phép ở trạng thái hiện tại. Vui lòng liên hệ hỗ trợ hoặc chờ BE cập nhật.");
      } else {
      ...
      <div className="max-w-[1100px] mx-auto space-y-6 animate-in fade-in duration-400">
        ...
        <div className="bg-white rounded-2xl border border-slate-200/80 shadow-[0_1px_3px_rgba(0,0,0,0.04)] px-6 py-5">
          <div className="flex items-start gap-4">
            <div className={cn(
              "w-12 h-12 rounded-xl flex items-center justify-center text-[18px] font-bold shrink-0 shadow-sm overflow-hidden",
              !request.startup.logoUrl ? cn("bg-gradient-to-br text-white", avatarGradient) : "bg-white border border-slate-100"
            )}>
              {request.startup.logoUrl ? (
                <img src={request.startup.logoUrl} alt={request.startup.displayName} className="w-full h-full object-cover" />
              ) : (
                initial
              )}
            </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-200/80 shadow-[0_1px_3px_rgba(0,0,0,0.04)] px-6 py-5">
              ...
              {request.preferredSlots.length === 0 && request.slotProposals.length === 0 && (
                <div className="text-[13px] text-slate-400">
                  <p>Chưa có thời gian đề xuất nào.</p>
                </div>
              )}
              ...
              {request.status === "COMPLETED" && request.isPayoutEligible === true && report && (
                <div className="space-y-6 animate-in slide-in-from-bottom-2 duration-500">
                  ...
                  <div className="grid grid-cols-1 gap-5 text-left">
                     <ReportField label="Nội dung báo cáo" content={report.content} icon={FileText} />
                     <p className="text-[11px] text-slate-400">
                       Ghi nhận lúc {new Date(report.createdAt).toLocaleString("vi-VN")}
                     </p>
                  </div>
                </div>
              )}
