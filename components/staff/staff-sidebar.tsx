...
const NAV_GROUPS: {
  ...
  },
  {
    label: "Quản trị",
    items: [
      { icon: MessageSquare, label: "Khiếu nại & Tranh chấp", href: "/staff/complaints" },
      { icon: AlertCircle,   label: "Báo cáo sự cố",          href: "/staff/issue-reports" },
    ],
  },
  {
    label: "Vận hành",
...
export function StaffSidebar({ collapsed }: StaffSidebarProps) {
  ...
  const displayEmail = user?.email || "staff@aisep.vn";
        ...
        <Link href="/staff" className="flex items-center gap-3 group">
          ...
          {!collapsed && (
            <div className="flex flex-col leading-tight">
              ...
              <span className="text-[#878164] text-[10px] font-normal font-manrope tracking-wide">Workspace</span>
            </div>
          )}
        </Link>
      </div>
