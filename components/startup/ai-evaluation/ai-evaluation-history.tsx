...
export function AIEvaluationHistoryList({ reports, currentId, onSelect }: AIEvaluationHistoryProps) {
  return (
              ...
              {reports.map((report) => (
                <tr 
                  ...
                  className={cn(
                    ...
                    report.evaluationId === currentId ? "bg-[#eec54e]/5" : "hover:bg-slate-50 dark:hover:bg-slate-800"
                  )}
                >
                  <td className="px-8 py-5">
                    ...
                    <p className="text-[11px] text-slate-400 font-medium">{new Date(report.generatedAt).toLocaleTimeString("vi-VN")}</p>
                  </td>
                  ...
                  <td className="px-8 py-5 text-right">
                    <button 
                      onClick={() => onSelect(report.evaluationId)}
                      disabled={report.evaluationId === currentId}
